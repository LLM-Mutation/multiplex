...
| [EXTENDING.md](EXTENDING.md) | Add a new mutant-generation approach, execution backend, language, or LLM provider — exact checklists of files to touch. |
...
- *multiplex* mutates **one method/function** per run (Java or Python, set by
  `project.language`): extract it with tree-sitter, ask an LLM to generate
  mutants, splice each mutant back into the source file, run the project's tests,
  and record which mutants survive.
